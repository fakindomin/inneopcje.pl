...
export async function ensureSchema(pool) {
  ...
  `);
  // seed_queue may already exist from before this column was added.
  await pool.query(`ALTER TABLE seed_queue ADD COLUMN IF NOT EXISTS last_error TEXT`);
...
export async function finishRun(pool, runId, { status, published = 0, draft = 0, failed = 0, note = null }) {
  await pool.query(
    `UPDATE bot_runs
     SET status = $2, published_count = $3, draft_count = $4, failed_count = $5, note = $6, finished_at = now()
     WHERE id = $1`,
    [runId, status, published, draft, failed, note]
  );
}
